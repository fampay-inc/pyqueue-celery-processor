...
class Consumer(Thread):
    ...
    def __init__(self, queue, debug=False):
        ...
        self.running = True
        self.queue = queue
        LOGGER.info("Python celery queue consumer thread started")

    def run(self):
        LOGGER.info("Consumer for celery tasks is running")
        while self.running:
            self.process_queue()
        LOGGER.info("Consumer for celery tasks stopped")

    def pause(self):
        # pause polling queue for newer tasks
        LOGGER.info("Celery-Queue consumer paused")
        self.running = False
    ...
    def process_task(self, task_item, max_tries=10):
        ...
        LOGGER.info("consuming task: %s | %s | %s", task_func.__name__, args, countdown)
        ...
        for retry in range(max_tries):
            try:
                ...
                LOGGER.info("consumed task: %s | %s | %s | %s", task_func.__name__, args,
                ...
                if retry == max_tries - 1:
                    LOGGER.info("Task %s dropped, error in broker connection | %s", task_func.__name__, task_item)
                    if sentry_sdk:
                ...
                TASKS_PROCESS_FAILED.labels(task_name=task_func.__name__).inc()  # pragma: no cover
                LOGGER.exception("Error in invoking celery task %s | %s", task_func.__name__, task_item)
                raise ValueError(
